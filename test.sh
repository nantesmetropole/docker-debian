...
# =================================================================
test_only_root() {
    local not_root="$(find / -xdev -not -user root -print -quit)"
...
test_lang() {
    local output="$(ls /nowhere 2>&1)"
    assert_equal "$output" "ls: impossible d'accéder à '/nowhere': Aucun fichier ou dossier de ce type" 'ls output'
}
